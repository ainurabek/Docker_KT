<template>
  <v-container>
    <v-card>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="primary" class=" ml-4" @click="circuitsGo">Каналы</v-btn>
        <v-btn
          :disabled="isEditing"
          :loading="!loadingEd"
          @click="editSave"
...
export default {
  ...
  methods: {
    userGetSub() {
      this.idPOForEdit = JSON.parse(localStorage.getItem("idPOForEdit")).id;
      ...
      console.log(user, this.idPOForEdit);
    },
    ...
    initialize() {
      console.log(this.idPOForEdit);
      this.$http
        .get(`${this.url.baseUrl}/apps/opu/objects/lp/${this.idPOForEdit}/`, {
          headers: { Authorization: `Token ${localStorage.token}` },
        })
        .then((response) => {
    ...
    editSave() {
      ...
      console.log(
        ...
        this.editedItem.total_amount_channels,
        this.editedItem.total_amount_active_channels
      );
      this.$http
        .put(
          `${this.url.baseUrl}/apps/opu/objects/lp/edit/${index}/`,
          {
            ...
            // total_amount_channels: this.editedItem.total_amount_channels,
            // total_amount_active_channels: this.editedItem.total_amount_active_channels
          },
          {
            headers: {
              Authorization: `Token ${localStorage.token}`,
