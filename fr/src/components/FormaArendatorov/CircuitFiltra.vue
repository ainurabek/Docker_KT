    ...
    >
      <template v-slot:top>
        <v-toolbar flat color="white">
          <v-toolbar-title>Форма Арендаторов:Каналы</v-toolbar-title>
...
export default {
  ...
  methods: {
    ...
    save() {
      const idCreateCirc = this.desserts[this.editedIndex].id;
      console.log(idCreateCirc);
      ...
      this.$http
        .post(
          `${this.url.baseUrl}/apps/opu/form_customer/api/circuit/create/${idCreateCirc}/`,
          formData,
          {
            headers: {
    ...
    initialize() {
      let idCirc = this.date;
      this.$http
        .get(
          `${this.url.baseUrl}/apps/opu/form_customer/api/circuit/${idCirc}/`,
          {
