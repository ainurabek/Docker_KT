...
export default {
  ...
  watch: {
    ...
    editedItemObjectProps(newValue) {
      if (newValue != {}) {
        this.initialize();
      }
    },
  ...
  methods: {
    initialize() {
      let tpo = this.editedItemTpoProps;
      let object = this.editedItemObjectProps;
      let point = this.editedItemPointProps;

      console.log(this.editedItemNameProps);
      this.$http
        .get(
          `${this.url.baseUrl}/apps/opu/objects/ip/?point_id=${point}&object_id=${object}&tpo_id=${tpo}`,
